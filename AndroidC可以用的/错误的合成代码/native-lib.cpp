...
list<int> get_bar_space_width(Mat img, int row) {
    int currentPix = -1;
    int lastPix = -1;
    int pos = 0;
    int i, j;
...
//    cvtColor(temp, imgGray, COLOR_RGBA2GRAY);
    ...
    //ostu
    imageBinary = Ostu(temp);


    for(int row = 0; row < temp.rows ; row++){
        width.clear();
        similarEdge.clear();
        barCode_string.clear();
        barCode.clear();
        barSpaceWidth.clear();
        //提取条空宽度
        width = get_bar_space_width(imageBinary, row);
        LOGE("tiaokuang:%d", width.size());
        if(width.size() >= 60){
            i = 0;
            for (list<int>::iterator iter = width.begin(); iter != width.end(); ++iter, i++) {
                barSpaceWidth[i] = *iter;
            ...
            //相似边译码
            barCode_string = decode_similar_edge(similarEdge);
            if(barCode_string.size() == 0){
                continue;
            }
            //将barCode的string
            ...
            decode_sharp(barCode_string, barSpaceWidth);
            //校验位
            if(check_bar_code(barCode)){
                break;
            }
        }
        LOGE("tupian:%d, %d",temp.rows,temp.cols);
